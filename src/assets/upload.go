...
func UploadAsset(repoName, filePath, directory string) *cerr.CustomError {
	format, uploadErr := uploadAssetNoReindex(repoName, filePath, directory)
	if uploadErr != nil {
		return uploadErr
	}
...
// UploadAssetBatch uploads a single asset without triggering a reindex. Bulk callers (e.g.
// `repo migrate`, which uploads many assets to the same target repo in a row) should use this
// and trigger a single reindex themselves once the whole batch is done, rather than paying for
// a reindex after every individual file.
func UploadAssetBatch(repoName, filePath, directory string) *cerr.CustomError {
	_, uploadErr := uploadAssetNoReindex(repoName, filePath, directory)
	return uploadErr
}

func uploadAssetNoReindex(repoName, filePath, directory string) (string, *cerr.CustomError) {
